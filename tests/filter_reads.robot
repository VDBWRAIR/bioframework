...
40 Quality doesn't change file
        Run Shell       ${filterIndex} ${in_fastq} -o ${testout2}.fastq --minimum 0 --index1 ${index1} --index2 ${index2} -o ${testout2}
        Files Should Be Equal   ${testout2}.fastq       ${in_fastq}

High maximum N's doesn't change file
    Run Shell      ${dropNs} ${in_fastq} -p -o ${testout1}.fastq -t --max-n 999
    Files Should Be Equal   ${testout1}.fastq       ${in_fastq}

filter out Ns
    Run Shell      ${dropNs} ${in_fastq} -p -o ${testout1}.fastq -t --max-n 1
    Run Shell       ${filterIndex} ${testout1}.fastq -o ${testout2}.fastq --minimum 32 --index1 ${index1} --index2 ${index2} -o ${testout2}
