...
convert_format file to stdout
    ${rc}    ${stdout} =            Run JIP Tool And Return RC And Output    ${tool} ${in_fastq} --out-format fasta
    Should Be Equal As Strings      ${stdout}        ${fastaout}

convert_format file to file
    ${rc}    ${stdout} =            Run JIP Tool And Return RC And Output    ${tool} ${in_fastq} -o ${testout}.fasta
...
convert_format missing outformat
    ${rc}    ${stdout} =            Run JIP Tool And Return RC And Output    ${tool} ${in_fastq}     expected_rc=1
